...
import { VideoEventPattern, VideoEventMessage, Video } from '@octonyah/shared-videos';
...
export class VideoEventsListener extends EventListenerBase {
  ...
  async handleVideoCreated(
    ...
    await this.processVideoForIndexing('created', data.video, context);
  }
  ...
  async handleVideoUpdated(
    ...
    await this.processVideoForIndexing('updated', data.video, context);
  }
  ...
  private async processVideoForIndexing(
    action: 'created' | 'updated',
    video: Partial<Video>,
    context: RmqContext,
  ) {
    this.logger.log(`Video ${action} event received: ${video.id}`);
    await this.videoIndexQueue.enqueueVideo(video?.id);
    await this.invalidateCache(video?.id);
    this.ack(context);
  }
